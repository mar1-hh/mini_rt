...
{
    float k = vars->radius / vars->height;
    k = k * k;
    
    vars->a = dot(ray_direction, ray_direction) - (1.0f + k) * vars->vdot_axis * vars->vdot_axis;
    vars->b = 2.0f * (dot(ray_direction, vars->oa) - (1.0f + k) * vars->vdot_axis * vars->oa_dot_axis);
    vars->c = dot(vars->oa, vars->oa) - (1.0f + k) * vars->oa_dot_axis * vars->oa_dot_axis;
