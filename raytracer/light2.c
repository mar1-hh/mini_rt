    ...
    t_vec3 normal, t_color surface_color)
{
    while (s_l_data->light)
    {
        ...
        if (!is_shadow(data, *point, s_l_data->light_dir_n, s_l_data->light->origin))
        {
            ...
            s_l_data->spec = powf(fmax(dot(s_l_data->view_dir, s_l_data->reflect_dir), 0.0f), 
                        s_l_data->shininess);
...
int	handle_light_shadow(t_minirt *data, t_point *point, t_vec3 normal)
{
    ...
    surface_color = get_surface_color(point);
    process_all_lights(data, &s_l_data, point, normal, surface_color);
